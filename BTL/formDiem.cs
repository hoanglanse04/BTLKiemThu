    ...
    public partial class formDiem : Form
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                ...
                try
                {
                    // Lấy giá trị điểm từ các TextBox và chuyển thành số thực (double)
                    double diemCC = double.Parse(tbDiemCC.Text.Trim());
                    double diemGK = double.Parse(tbDiemGK.Text.Trim());
                    double diemThi = double.Parse(tbDiemThi.Text.Trim());

                    // Tính điểm trung bình cộng theo công thức yêu cầu
                    double diemTBC = (diemCC * 0.2) + (diemGK * 0.3) + (diemThi * 0.5);
                    ...
                    // Cập nhật lại GridView và làm mới form
                    Load_DB_toGridView();
                    refresh();
                }
                catch (Exception err)
